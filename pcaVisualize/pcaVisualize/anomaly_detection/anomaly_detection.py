...
def createSpark():
    spark_session = SparkSession.builder.appName("PCA").getOrCreate()
    return spark_session

# The path of data and the path of outputs are declared.
def obtain_worked_path():
...
def readData(dataFileName, sparkSession):
    df = sparkSession.read.option("header", "true").option("inferSchema", "true").csv(
         dataFileName + ".csv")
    return df
...
"""
def obtain_anomaly_values(dataFrame, selected_column_names, confidence_level, time_col_name):
    anomaly_value_indices = pca.anomalyDetectionWithPCA(dataFrame=dataFrame, selected_column_names=selected_column_names,
                                                        confidence_level=confidence_level,
                                                        time_column_name=time_col_name)
    return anomaly_value_indices
...
def obtain_anomaly_normal_values(df, anomaly_value_indices):
    ...
    return anomaly_column, normal_column

def return_pandas_data_frame(dataFrame, anomaly_column, selected_column_names):

    df_pandas = dataFrame.select(selected_column_names).toPandas()
    #print(df_pandas)
...
def obtain_anomaly_columned_spark_data_frame(sparkSession, anomaly_column, dataFrame):

    df_x4 = sparkSession.createDataFrame([Row(**{'Anomaly': x}) for x in anomaly_column])
    df = add_row_num(dataFrame)
...
def data_test_for_80_perc_train_20_perc_tests_data(sparkSession, df_pandas,selected_column_names, normal_column, anomaly_value_indices):
    ...
    pyspark_train_df = sparkSession.createDataFrame(df_train)
    ...
    result = []
    result.extend(logistic_regression_accuracy_summary)
...
def data_set_test_with_10_folds_cross_validation(sparkSession, selected_column_names, df_pandas, normal_column, anomaly_value_indices):
    ...
    result = []
    result.extend(ave_confusion_matrix)
    result.append(ave_accuracy_rate)
    result.append(ave_f_score)
